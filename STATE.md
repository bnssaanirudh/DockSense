...
- `scripts/eval_reasoning.py` + exact per-frame boxes from the renderer — the
  ...
  a held-out split of 27 clips: **F1 0.933**, with
  `no_tracking` collapsing to **0.000**. The tune split reads 1.000 and must not
  be quoted — see the claims ledger.
...
| When | Who | What |
|---|---|---|
| 10 Sep | Claude | **Real footage arrived (session 1, 7 clips, ~3 min) and the system met it for the first time.** The footage is not direct camera output — every clip is a phone recording of an NVMS playback window, so `configs/rois.yaml` + `scripts/prepare_footage.py` crop each frame to the camera pane before anything sees it. Detector works on it (cartons, people, trolleys correctly boxed) but only after dropping `conf_threshold` 0.25 -> 0.10, chosen on the three TUNE clips: two lossy encodes plus screen moire depress confidence without making the boxes wrong. Real footage then exposed three things no rendered clip could: (1) `min_track_age_frames` was **declared in behaviours.yaml and read by nothing** — a brand-new track's first velocity is a full-magnitude jump, which on soft footage is detector jitter, and that jitter was firing throws on clips of people dragging; now enforced in `FeatureExtractor`. (2) **A single horizontal floor line cannot describe a perspective view** — one line puts a box on the floor at one depth and a metre up at another, which is why B03 never fired; floor gap is now measured against the nearest person's feet, a calibration-free local ground plane. (3) B02 counted qualifying frames **anywhere** in its lookback rather than as a run, so one jitter frame satisfied "flight"; now a consecutive run. Clip-level results and what is still broken: `artifacts/evaluation/real_footage.md`. |
| 10 Sep | Claude | **Built a held-out split, because F1 1.000 was a warning, not a result.** The renderer now samples scenario parameters instead of hard-coding them: `tune` is one fixed draw (9 clips, what development happened against), `heldout` is three fresh draws per scenario (27 clips) with different release heights, launch speeds, drag directions, stack offsets and **carton sizes from 60 to 100 px** — the last one specifically to catch any threshold that is secretly in pixels rather than object-heights. Scored: **held-out F1 0.875** against **1.000** on tune. That 0.125 gap is the size of the overfit. (Re-scored later the same day at 0.933 after real-footage-driven changes — see the ledger; a set scored twice is no longer strictly held out.) Two ablation rows now carry real signal: `no_tracking` 0.000 on both splits, and `no_smoothing` **0.875 → 0.812 on held out while showing no delta at all on tune** — the fixed clips had no jitter for smoothing to remove, which is exactly what a single fixed set hides. Three held-out failures left **deliberately unfixed** (track ID switch on a fast throw, a high carry reading as a throw, one slow drag under threshold): tuning them away after seeing them would turn the held-out split into a second tuning set. |
...
| **Reasoning-layer precision / recall / F1 (HELD OUT)** | ✅ 10 Sep | `artifacts/evaluation/reasoning_eval_heldout.json`, 27 clips / 15 labelled events / 12 hard negatives, exact perception injected, scored once | **0.933 / 0.933 / 0.933** — scored TWICE (0.875 first, then 0.933 after real-footage-driven changes), so no longer strictly held out; say so. Same renderer as the tune split, so it controls for threshold overfitting but NOT for generator bias |
| **Ablation: no_tracking** | ✅ 10 Sep | same | **F1 0.933 → 0.000** on held out (and 1.000 → 0.000 on tune). Without persistent identity nothing fires at all. The hardest row to game — lead with it |
| **Ablation: no_smoothing** | ✅ 10 Sep | same | **F1 0.933 → 0.867** on held out; **no delta at all** on the tune split. Varying size and speed is what makes smoothing earn its place |
| Reasoning F1 on the **tune** split | ⚠️ 10 Sep | `reasoning_eval_tune.json` | **1.000 — do not quote.** Thresholds and detector rules were changed in response to failures on these nine clips. Kept only as the contrast that sizes the overfit |
| Ablation: no_event_graph | ✅ 10 Sep | same | **no delta on either split.** Reported, not hidden: the graph feeds the risk score, not event detection, so event F1 is the wrong instrument for it |
| **Behaviour presence on real footage** | ✅ 10 Sep | `artifacts/evaluation/real_footage.md`, session 1, 7 clips, clip level | **1 of 9 expected behaviours reported.** Poor, and the most useful number here. Quote it before any synthetic figure |
| Precision on **real** footage | ❌ **NOT MEASURABLE** | — | Session 1 has **no hard-negative clip**. Every clip is a positive, so a detector that fired constantly would score perfectly. One 60 s clip of normal handling fixes this |
