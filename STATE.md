...
- `scripts/eval_reasoning.py` + exact per-frame boxes from the renderer — the
  ...
  a held-out split of 27 clips: **F1 0.875** (P 0.824, R 0.933), with
...
| **Reasoning-layer precision / recall / F1 (HELD OUT)** | ✅ 10 Sep | `artifacts/evaluation/reasoning_eval_heldout.json`, 27 clips / 15 labelled events / 12 hard negatives, exact perception injected, scored once | **0.824 / 0.933 / 0.875** — quote this one. Same renderer as the tune split, so it controls for threshold overfitting but NOT for generator bias |
| **Ablation: no_tracking** | ✅ 10 Sep | same | **F1 0.875 → 0.000** on held out (and 1.000 → 0.000 on tune). Without persistent identity nothing fires at all. The hardest row to game — lead with it |
| **Ablation: no_smoothing** | ✅ 10 Sep | same | **F1 0.875 → 0.812** on held out; **no delta at all** on the tune split. Varying size and speed is what makes smoothing earn its place |
